...
class McMadness(commands.Cog):
    def __init__(self, client) -> None:
        ...
        self.game_cache = {"message": None, "answer": None, "easy": 0, "normal": 0, "hard": 0, "embed": None}
    ...
    async def clear_variables(self):
        self.game_cache = {"message": None, "answer": None, "easy": 0, "normal": 0, "hard": 0}
        self.users_guessed = []
        self.members_ready = False
        self.question_start_ts = 0
        self.used_questions = []
    ...
    async def join_event(self, channel):
        ...
        await msg.edit(embed=msg.embeds[0], components=action_row)
    ...
    async def main_event_func(self):
        ...
        while 1:
            if self.game_cache["easy"] + self.game_cache["normal"] + self.game_cache["hard"] == 280:
            ...
            reminder.set_author(name="10 SECONDS LEFT TO GUESS")
    ...
    # Check for users that didn't try...
            did_not_try = []
            for id, pts in self.all_game_participants.items():
            ...
            if winner_id:
                ...
                if self.is_tournament:
                    text += f"\n\nAs the sole, you will receive <:money:903467440829259796> **{self.prize_pool:,}**!"
                    # await self.client.addcoins(winner_id, self.prize_pool)
                else:
                    if self.give_10k_to_winner:
                        text += f"\n\nAs the sole, you will receive <:money:903467440829259796> **10,000**!"
                        # await self.client.addcoins(winner_id, 10000)

                win_em = discord.Embed(title="Game Over!", description=text)
    ...
    async def tournamet(self):
        ...
        self.prize_pool = len(self.prize_pool) * 10000
    ...
    async def return_difficulty(self):
        if self.game_cache["easy"] < 30 and self.game_cache["normal"] == 0 and self.game_cache["hard"] == 0:
            self.game_cache["easy"] += 1
            return "easy"        

        elif self.game_cache["easy"] == 30 and self.game_cache["normal"] == 0 and self.game_cache["hard"] == 0:
            self.game_cache["normal"] += 1
            return "normal"
        
        if self.game_cache["normal"] < 10 and self.game_cache["easy"] == 30 and self.game_cache["hard"] == 0:
            self.game_cache["normal"] += 1
            return "normal"
        elif self.game_cache["normal"] == 10 and self.game_cache["easy"] == 30 and self.game_cache["hard"] == 0:
            self.game_cache["hard"] += 1
            return "hard"

        if self.game_cache["hard"] < 10 and self.game_cache["easy"] == 30 and self.game_cache["normal"] == 10:
            self.game_cache["hard"] += 1
            return "hard"

        elif self.game_cache["hard"] == 10 and self.game_cache["easy"] == 40 and self.game_cache["normal"] == 10:
    ...
    async def prepare_question(self):
        ...
        icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
        em.set_footer(text="⏳ Try to answer it within 30 seconds", icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
        
        action_row = [manage_components.create_actionrow(*option_buttons)]

        return em, action_row
    ...
    async def handle_incorrect_answer(self, ctx):
        if ctx.author_id in self.users_guessed and ctx.author_id in self.participants:
            em = discord.Embed(color=0x00F8EF)
            em.set_author(name="You already answered this question.",
            icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
            return await ctx.send(embed=em, hidden=True)

        if ctx.author_id in self.participants and ctx.author_id not in self.users_guessed:
