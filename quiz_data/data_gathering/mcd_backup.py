...
class McMadness(commands.Cog):
    def __init__(self, client) -> None:
        self.client = client
        self.unbelievaboat_api_enabled = False
    ...
    async def check_for_monthly_pay(self):
        ...
        scheduled_pay_time = datetime.strptime("2022-02-01 00:01:00", "%Y-%m-%d %H:%M:%S").timestamp()
        ...
        if datetime.utcnow().timestamp() >= scheduled_pay_time:
            ...
            for index in range(9):
                ...
                mem = guild.get_member(int(data[index][0]))

                em = discord.Embed(colro=0x78BB67, description=f"<:Checkmark:886699674277396490> Added <:money:903467440829259796>**{payouts[0]:,}** to {mem.mention}'s bank balance.")
                em.set_author(name="Top 10 Weekly Messagesrs Payout", icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
                
                await ch.send(embed=em)

                print(f"Added {payouts[0]} coins to {mem.name} | {self.client.po_data['mm_tournament'][data[index][0]]}")
                payouts.pop(0)
    ...
    async def update_tournament_lb(self, id:int=None):
        if "mm_tournament" not in self.client.po_data.keys():
            ...
            self.client.po_data["mm_tournament"] = 1
        else:
            self.client.po_data["mm_tournament"][str(id)] += 1
    ...
    async def update_tournament_lb(self, id:int=None):
        if "mm_casual" not in self.client.po_data.keys():
            ...
            self.client.po_data["mm_casual"] = 1
        else:
            self.client.po_data["mm_casual"][str(id)] += 1
    ...
    async def main_event_func(self):
        ...
        while 1:
            ...
            main_em, ar, sleep_time = await self.prepare_question()  # ar == action_row (3 buttons)
    ...
    # Check for users that didn't try...
            ...
            for id, pts in self.participants.copy().items():
                if id not in self.users_guessed:
                    ...
                    try:
                        ...
                        print(f"Tried to delete {id} in line 195, failed.")
            self.last_disqualified_team = did_not_try

            winner_id = 0; winner_points = 0

            if not did_not_try and len(self.participants) == 0 and self.last_disqualified_team:
                winner_id, winner_points = sorted(self.last_disqualified_team, key=lambda r: r[1], reverse=True)[0]
                print("I tried qwq", self.last_disqualified_team)

            elif len(self.participants) == 0:
                winner_id, winner_points = sorted(did_not_try, key=lambda r: r[1], reverse=True)[0]
            ...
            if winner_id:
                ...
                if self.is_tournament:
                    ...
                    await self.update_norm_lb(winner_id)
                    if self.give_10k_to_winner:
    ...
    # Check if there's 1 player left in the game.
            ...
            if len(self.participants) == 1:
                ...
                if self.is_tournament:
                    await self.update_tournament_lb(solo_win_id) 
                    text += f"\n\nAs the sole survivor, you will receive <:money:903467440829259796> **{self.prize_pool:,}**!"
                    if self.unbelievaboat_api_enabled:
                        await self.client.addcoins(solo_win_id, self.prize_pool)
                else:
                    await self.update_norm_lb(solo_win_id)
                    if self.give_10k_to_winner:
    ...
    async def __messages_(self, member:discord.Member=None):
        ...
        em.set_author(name=f"{member.name}'s Minecraft Madness Stats:", icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
    ...
    async def __top_(self, ctx, tournament_or_casual):
        ...
        em = discord.Embed(color=0x00F8EF, title=f"Minecraft Madness ({tournament_or_casual.lower().capitalize()}) Leaderboard")
        em.set_author(name="\u200b", icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))

        if not "mm_month_count" in self.client.po_data.keys():
            self.client.po_data["mm_month_count"] = 1

        if (("mm_casual" not in self.client.po_data.keys() or not self.client.po_data["mm_casual"]) and tournament_or_casual.lower() == "casual") or (("mm_tournament" not in self.client.po_data.keys() or not self.client.po_data["mm_tournament"]) and tournament_or_casual.lower() == "tournaments"):  # If there are no memebrs with that role, return an embed with the number 0 as the user count.
        ...
        if (
            ...
            if tournament_or_casual == "tournaments":
                data = enumerate(sorted([[key, self.client.po_data["mm_tournament"][key]] for key in self.client.po_data["mm_tournament"].keys()], key=lambda e: e[1], reverse=True))
            else:
                data = enumerate(sorted([[key, self.client.po_data["mm_casual"][key]] for key in self.client.po_data["mm_casual"].keys()], key=lambda e: e[1], reverse=True))

            users = ""

            for user in data:
                mem = ctx.guild.get_member(int(user[1][0]))

                if user[0] == 0:
                    users += f"🏆 - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 500k"
                
                elif user[0] == 1:
                    users += f"\n🥈 - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 400k"
               
                elif user[0] == 2:
                    users += f"\n🥉 - **{mem.name['name']}** ({user[1][1]} wins) - <:money:903467440829259796> 300k"
                
                elif user[0] == 3:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 200k"
               
                elif user[0] == 4:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 100k"
                
                elif user[0] == 5:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 80k"
                
                elif user[0] == 6:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 60k"
               
                elif user[0] == 7:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 40k"
               
                elif user[0] == 8:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 20k"
                
                elif user[0] == 9:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins) - <:money:903467440829259796> 10k"
               
                else:
                    users += f"\n**{user[0]}.** - **{mem.name}** ({user[1][1]} wins)"
        ...
        # If there are more than 10 members for that role, create paginator and send embeds with 10 memebrs per embed.
            ...
            if tournament_or_casual == "tournaments":
                data = sorted([[key, self.client.po_data["mm_tournament"][key]] for key in self.client.po_data["mm_tournament"].keys()], key=lambda e: e[1], reverse=True)
            else:
                data = sorted([[key, self.client.po_data["mm_casual"][key]] for key in self.client.po_data["mm_casual"].keys()], key=lambda e: e[1], reverse=True)
            ...
            for index, key in enumerate(data):  # x == member
                ...
                mem = ctx.guild.get_member(key[0])
                ...
                if len(temp_li) == 10:
                    ...
                    users = "\n".join(f"**{user[0]}** - **{ctx.guild.get_member(user[1][0]).name}** ({user[1][1]} wins)" for user in temp_li)
    ...
    guild_ids=guild_ids, 
     ...
     options=[
        create_option('tournaments_or_casual', 'The leaderboard type to view: Casual or Tournaments leaderboard', 3, False),
    ])
    async def _mm_lb(self, ctx: SlashContext, tournaments_or_casual:str=None):
        ...
        if not tournaments_or_casual:
            ...
            ("t" not in tournaments_or_casual.lower()) and ("c" not in tournaments_or_casual.lower())
        ):
            try:
