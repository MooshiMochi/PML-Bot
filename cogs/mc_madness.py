...
class McMadness(commands.Cog):
    ...
    async def check_for_monthly_pay(self):
        ...
        scheduled_pay_time = datetime.strptime("2022-02-01 05:01:00", "%Y-%m-%d %H:%M:%S").timestamp()
        ...
        if datetime.now().timestamp() >= scheduled_pay_time:
            ...
            next_schedule = ""    
            if int(_date[5:7]) > 12:
                next_schedule = datetime.strptime(str(int(str(datetime.now().date())[:4])+1) + "-01-01 05:01:00", "%Y-%m-%d %H:%M:%S").timestamp()
            else:
                ...
                next_schedule = datetime.strptime(str(datetime.now().date())[:5] + filler + str(int(str(datetime.now().date())[5:7])+1) + "-01 05:01:00", "%Y-%m-%d %H:%M:%S").timestamp()
            ...
            if data:
                for index in range(10):
                    ...
                    print(f"Added {payouts[0]} coins to {mem} - {data[index][0]} | {self.client.po_data['mm_tournament'][data[index][0]]}")
                    payouts.pop(0)
    ...
    async def __messages_(self, member:discord.Member=None):
        ...
        em.set_author(name=f"{the_name}'s Minecraft Madness Stats:", icon_url=self.client.user.avatar_url_as(static_format="png", size=2048))
